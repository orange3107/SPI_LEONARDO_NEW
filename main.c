...
int i;
  ...
  freq = strtol(argv[1], NULL, 0);
	...
	rc = spilcdInit(LCD, 0, 0, freq, 12, 7, -1); // LCD type, flip 180, SPI Channel, D/C, RST, LED
	if (rc != 0)
	{
  ...
  static unsigned char bmpBuffer[2][(240 * 320) * 3];
  
  unsigned char *test = imageFromFile("test.bmp", bmpBuffer[0]);
  
  unsigned char *test1 = imageFromFile("test1.bmp", bmpBuffer[1]);
  
  
  
  while(1){
  
    drawImage(test, 0, 0, 240, 320);
    drawImage(test1, 0, 0, 240, 320);
