...
//unsigned char bmpBuffer[(240 * 320) * 3];

 unsigned char *imageFromFile(char path[], unsigned char *bmpBuffer){
  ...
  fseek(pFile, 54, 0);
	fread(bmpBuffer, pixelSize, 240 * 320, pFile);
  fclose(pFile);
...
void drawImage(unsigned char *bmpBuffer, int x, int y, int w, int h) {
	...
	uint8_t* buffer = (uint8_t*)malloc(w * 2 * 5);
  ...
  int f = 0;
	...
	//spilcdWriteDataBlock(buffer, h*w*2);
	//buffer  = NULL;
